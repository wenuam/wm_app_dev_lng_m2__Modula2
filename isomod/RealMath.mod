...
PROCEDURE poly(p : ARRAY OF LONGREAL; x : LONGREAL; n : ADRINT) : LONGREAL;
VAR
    i   : ADRINT;
    s   : LONGREAL;
BEGIN
...
%IF IA32 %THEN
PROCEDURE fexp(x : REAL) : REAL; ASSEMBLER;
%IF Bits32 %THEN
ASM
        FLD     x
        FLD1
...
PROCEDURE fln(x : REAL) : REAL; ASSEMBLER;
ASM
...
BEGIN
	...
	%ELSE
    	IF x > 0.0 THEN
	        ...
	        z := (lnz - 1.0) / (lnz + 1.0);
    	    zsq := z * z;
        	lnz  := (z * poly(lnp, zsq, 2)) / poly(lnq, zsq, 3);
...
%IF IA32 %THEN
PROCEDURE fsin(x : REAL) : REAL; ASSEMBLER;
ASM
        FLD     x
        FSIN
END fsin;
...
BEGIN
    ...
    %ELSE
		...
		IF which[oct] = dosin THEN
			sx := sx * poly(sinpoly, sx*sx, 6);
		ELSE
			sx := poly(cospoly, sx*sx, 6);
		END;
...
%IF IA32 %THEN
PROCEDURE fcos(x : REAL) : REAL; ASSEMBLER;
ASM
        FLD     x
        FCOS
END fcos;
...
BEGIN
    ...
    %ELSE
		...
		IF which[oct] = dosin THEN
			cx := cx * poly(sinpoly, cx*cx, 5);
		ELSE
			cx := poly(cospoly, cx*cx, 5);
		END;
...
PROCEDURE ftan(x : REAL) : REAL; ASSEMBLER;
ASM
        FLD     x
        FSINCOS
        FDIVP
...
BEGIN
    ...
    %ELSE
		...
		xsq := tx * tx;
		tx := tx * poly(tanp, xsq, 4) / poly(tanq, xsq, 3);
...
PROCEDURE farctan(x : REAL) : REAL; ASSEMBLER;
ASM
        FLD     x
        FCOM    zero
        FSTSW       AX
...
BEGIN
    ...
    %ELSE
		...
		atx := (atx - r) / (1.0 + (atx * r));
		atx := artannover16[i] + (atx * poly(atp, atx*atx, 5));
