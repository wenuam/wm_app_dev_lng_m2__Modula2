...
BEGIN
    ...
    RealToFloat(real, sigFigs, temp); (* convert REAL number to a string *)
...
BEGIN
    ...
    RealToEng(real, sigFigs, temp); (* convert REAL to a string *)
    IF LENGTH(temp) < width THEN
        (* write spaces *)
...
BEGIN
    ...
    RealToFixed(real, place, temp); (* convert REAL to fixed *)
    IF LENGTH(temp) < width THEN
        (* write spaces *)
...
BEGIN
    ...
    IF ABS(real) >= 1.0 THEN
        (* get the number of digits in the whole part *)
